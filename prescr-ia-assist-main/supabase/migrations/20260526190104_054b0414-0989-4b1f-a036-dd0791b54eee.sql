...
AS $$
  ...
  dups AS (
    ...
    FROM (
      SELECT COUNT(*) c FROM m
      GROUP BY nom_k, dosage_k, forme_k
      HAVING COUNT(*) > 1
    ) g
  ),
  pc_per_med AS (
    SELECT m.nom_k, COUNT(v.id) AS pc_count
    FROM m
    LEFT JOIN public.medicament_pc_valide v ON v.medicament_id = m.id
    GROUP BY m.nom_k
  ),
  pc_stats AS (
    SELECT
...
GRANT EXECUTE ON FUNCTION public.get_medicaments_coverage_stats() TO authenticated, service_role;
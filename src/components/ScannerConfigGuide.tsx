...
import * as ScannerCodes from "@/lib/scannerConfigCodes";
import type { ScannerFamily, ScannerStep } from "@/lib/scannerConfigCodes";

// Tolère à la fois le nouveau nom (SCANNER_FAMILIES) et l'ancien (SCANNER_MODELS)
// pour éviter de casser le build si un seul des deux fichiers est synchronisé.
const SCANNER_FAMILIES: ScannerFamily[] =
  (ScannerCodes as any).SCANNER_FAMILIES || (ScannerCodes as any).SCANNER_MODELS || [];
const STATUS_LABEL = ScannerCodes.STATUS_LABEL;
    ...
    <div className="space-y-4">
      <div className="text-xs text-muted-foreground leading-relaxed bg-secondary/50 rounded-lg p-3 space-y-1">
        ...
        <p>
          ...
          via 6 chemins passifs simultanés — aucun mode spécial n'est requis sur le scanner.
        </p>

...
USING (
  CASE
    WHEN realtime.topic() LIKE 'scan_queue_%' THEN
      public.current_user_pharmacy_id() IS NOT NULL
      AND realtime.topic() = ('scan_queue_' || public.current_user_pharmacy_id()::text)
    ELSE false
  END
);
